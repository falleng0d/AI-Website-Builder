...
import { SandboxProvider, SandboxInfo, CommandResult, SandboxProviderConfig, SandboxConfig } from "../types";

export class VercelProvider extends SandboxProvider {
  private existingFiles: Set<string> = new Set();
  private vercelConfig: SandboxProviderConfig["vercel"];
  ...
  async createSandbox(config?: SandboxConfig): Promise<SandboxInfo> {
    try {
      ...
      // Add authentication based on config passed from factory
      if (this.vercelConfig?.token && this.vercelConfig?.teamId && this.vercelConfig?.projectId) {
        sandboxConfig.teamId = this.vercelConfig.teamId;
        sandboxConfig.projectId = this.vercelConfig.projectId;
        sandboxConfig.token = this.vercelConfig.token;
      }
  ...
  async installPackages(packages: string[]): Promise<CommandResult> {
    ...
    const flags = this.config.npmFlags || "";
    ...
    if (result.exitCode === 0 && this.config.autoRestartVite === true) {
